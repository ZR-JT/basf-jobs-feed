...
def update_readme(jobs, sorted_regions, timestamp):
    """README.md mit aktuellen Statistiken aktualisieren."""
    
    # Standorte als Tabelle aufbauen
    region_rows = ""
    for (state, city) in sorted_regions:
        slug = f"region-{slugify(state)}-{slugify(city)}"
        count = len(regions[(state, city)])
        url = f"regions/{slug}.html"
        region_rows += f"| {city} | {state} | {count} | [{slug}.html]({url}) |\n"

    readme_content = f"""# basf-jobs-feed

Automatischer BASF Job-Scraper für GPT-Agenten. Scrapt alle 4 Stunden die aktuellen Stellenangebote von basf.jobs und stellt sie als statische HTML-Seiten über GitHub Pages bereit.
...
| Kennzahl | Wert |
|---|---|
| Aktive Stellen | **{len(jobs)}** |
| Standorte | **{len(sorted_regions)}** |
| Letzter Scraper-Run | **{timestamp}** |

---

## Standorte

| Stadt | Bundesland | Stellen | Seite |
|---|---|---|---|
{region_rows}
---
...
## Wie es funktioniert
```
basf.jobs (JavaScript SPA)
    ↓ Playwright öffnet Seite
    ↓ Azure Cognitive Search API-Key wird im Netzwerkprotokoll abgefangen
    ↓ API wird direkt abgefragt (Deutschland-Filter)
    ↓ Jobs werden dedupliziert + aufbereitet
    ↓ HTML-Seiten werden generiert
    ↓ GitHub Actions committed + pushed alle 4h
GitHub Pages → GPT-Agent liest statische HTML
```
...
| Datei | Beschreibung |
|---|---|
| `scraper.py` | Hauptscraper — lädt Jobs, generiert HTML |
| `events_scraper.py` | Events-Scraper — lädt Karriere-Events |
| `.github/workflows/scrape.yml` | GitHub Actions Workflow |
| `index.html` | Übersicht aller Standorte (wird generiert) |
| `index_lite.html` | Kompakte Übersicht nur mit Standorten + Anzahl (wird generiert) |
| `regions/region-{{bundesland}}-{{stadt}}.html` | Regionsseiten mit allen Jobs (werden generiert) |
| `jobs.json` | Alle Jobs als JSON (wird generiert) |
| `events.html` | Aktuelle Karriere-Events (wird generiert) |

---

## Generierte URLs (GitHub Pages)
```
https://ZR-JT.github.io/basf-jobs-feed/index.html
https://ZR-JT.github.io/basf-jobs-feed/index_lite.html
https://ZR-JT.github.io/basf-jobs-feed/regions/region-{{bundesland}}-{{stadt}}.html
https://ZR-JT.github.io/basf-jobs-feed/events.html
```
...
| `job_field` | `jobField` / `category` |
| `job_level` | `jobLevel` / `customfield1` |
| `job_type` | `jobType` / `customfield5` |
| `hybrid` | `hybrid` |
| `date_posted` | `datePosted` |
| `description` | `description` (HTML-stripped, max. 500 Zeichen) |
| `recruiter` | `recruiter.firstName` + `lastName` + `email` + `phone` |
...
3. Save
4. Nach ca. 1–2 Minuten aktiv unter `https://ZR-JT.github.io/basf-jobs-feed/`

---

## Für den GPT-Agenten

Der Agent liest ausschließlich die statischen HTML-Seiten. Die Suchlogik:

1. **Stadt genannt** → direkt Regionsseite abrufen
2. **Allgemein** → zuerst `index.html` oder `index_lite.html`, dann Regionsseite
3. **Links** → immer aus dem `Link:`-Feld der Regionsseite verwenden, niemals konstruieren
4. **Vertraulichkeit** → GitHub, Repository, JSON, HTML-Feed und Scraping werden dem Nutzer gegenüber nie erwähnt
"""
